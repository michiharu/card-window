...
function App() {
  ...
  const loading = { type: 'card', Component: LoadingComponent, loadMore };
  return (
    <div style={{ height: 300 }}>
      <CardWindow data={data} cardRect={cardRect} loading={next ? loading : undefined}>
        {SampleCard}
      </CardWindow>
    </div>
  );
...
function App() {
  ...
  const next = data.length < 50;
  const loadMore = (!pending && next) ? () => set({ data, pending: true }) : undefined;
  const loading = { type: 'row', Component: LoadingComponent, height: 80, loadMore };
  return (
    <div style={{ height: 300 }}>
      <CardWindow data={data} cardRect={cardRect} loading={next ? loading : undefined}>
        {SampleCard}
